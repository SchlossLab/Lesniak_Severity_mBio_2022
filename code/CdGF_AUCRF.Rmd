...
aucrf_data <- Predict_early_euth_df[,c('Euth_Early',euth_otus)]
...
#### leave-one-cage-out
LOcO_df <- Predict_df %>% 
                select(Euth_Early, cage_id, one_of(euth_otus))
...
```{r importance, fig.width=10, fig.height=10}
...
for (i in seq_along(euth_otus)) {
  partialPlot(rf_early_aucrf$RFopt, aucrf_data, euth_otus[i], xlab='',
