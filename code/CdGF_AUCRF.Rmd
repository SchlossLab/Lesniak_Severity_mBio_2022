...
LOcO_probs <- NULL
LOcO_probs <- sapply(1:length(cage_list), function(i){
  ...
  set.seed(seed)
  temp_model <- AUCRF(Euth_Early~., data=train, pdel=0.99, ntree=500)
  LOcO_probs[i] <- predict(temp_model$RFopt, test, type='prob')[,2]
})
LOcO_probs <- unlist(LOcO_probs)
LOcO_roc <- roc(aucrf_data$Euth_Early~LOcO_probs)
