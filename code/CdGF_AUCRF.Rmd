...
all_mice_probs <- data.frame(obs = Predict_early_euth_df$Euth_Early,
                             pred = otu_euth_probs[,2])

calibration_plots <- function(prediction_df){
  # prediction_df must have obs in col 1 and predict in col 2
  histo_plot <- ggplot(data.frame(prediction_df), aes(x=pred)) + 
    ...
    theme_bw()
  cal_plot <- data.frame(prediction_df,
                         bin = cut(prediction_df[,2], 
                                   breaks =c(-Inf,seq(0.1,0.9,0.1), Inf),
                                   labels = seq(0.05,0.95,0.1))) %>% 
    group_by(bin) %>% 
    summarise(positive_portion = sum(obs==1)/length(obs)) %>% 
...
all_mice_cal <- calibration_plots(all_mice_probs)
...
om_cal <- calibration_plots(om_probs)
...
source_list <- as.character(unique(LOsO_df$human_source))

LOsO_probs <- c()
LOsO_rf <- c()
for(i in 1:length(source_list)){
  source <- LOsO_df$human_source == source_list[i]
  LOsO_predict <- select(LOsO_df, -human_source)
  train <- LOsO_predict[!source,]
  test <- LOsO_predict[source,]
  set.seed(seed)
  temp_model <- AUCRF(Euth_Early~., data=train, pdel=0.99, ntree=500)
  LOsO_rf[[i]] <- temp_model$RFopt
  LOsO_probs[LOsO_df$human_source == source_list[i]] <- predict(temp_model$RFopt, test, type='prob')[,2]
}
LOsO_roc <- roc(LOsO_df$Euth_Early~LOsO_probs)
LOsO_cal <- calibration_plots(data.frame(obs=LOsO_df$Euth_Early,
                                         pred=LOsO_probs))
#### leave-2-out
LTsO_df <- Predict_df %>% 
                select(Euth_Early, human_source, one_of(om_import_otus))
donor_list <- as.character(unique(LTsO_df$human_source))
...
for(i in seq(1,length(donor_list),2)){
  ...
  by_donor <- LTsO_df$human_source %in% donors[i:(i+1)]
  LTsO_predict <- select(LTsO_df, -human_source)
  train <- LTsO_predict[!by_donor,]
  test <- LTsO_predict[by_donor,]
  set.seed(seed)
  temp_model <- AUCRF(Euth_Early~., data=train, pdel=0.99, ntree=500)
  LTsO_rf[[i]] <- temp_model$RFopt
  LTsO_probs[LTsO_df$human_source %in% donors[i:(i+1)]] <- predict(temp_model$RFopt, test, type='prob')[,2]
}
LTsO_roc <- roc(LTsO_df$Euth_Early~LTsO_probs)
LTsO_cal <- calibration_plots(data.frame(obs=LTsO_df$Euth_Early,
                                         pred=LTsO_probs))
...
om_loo_cal <- calibration_plots(om_loo_all)
...
plot(LOsO_roc, col='purple', lwd=2, add=T, lty=3)
plot(LTsO_roc, col='red', lwd=2, add=T, lty=3)
plot(om_loo_roc, col='blue', lwd=2, add=T, lty=2)
mtext(side=2, text="Sensitivity", line=2.5)
mtext(side=1, text="Specificity", line=2.5)
legend('bottomright', legend=c(sprintf('One-mouse (AUC: %.3g)',om_roc$auc),
                           sprintf('Leave-1 source-out (AUC: %.3g)',LOsO_roc$auc),
                           sprintf('Leave-2 sources-out (AUC: %.3g)',LTsO_roc$auc),
