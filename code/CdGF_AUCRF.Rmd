...
for(j in 1:iters){
  ...
  for(n in unique(as.character(mouse_set$human_source))){
    ...
    temp_model <- AUCRF(Euth_Early~., data=select(train, -mouse_id, -human_source)
                      , pdel=0.2, ntree=500, ranking = 'MDA')
    om_probs <- rbind(om_probs,
                      data.frame(obs = test$Euth_Early,
                                 pred = predict(temp_model$RFopt, test, type='prob')[,2]))
    om_cage_import <- c(om_cage_import, temp_model$Xopt)
  }
  om_roc <- roc(om_probs$obs~om_probs$pred)
  om_import_otus <- merge(om_import_otus,
                          data.frame(otu = names(table(om_cage_import)),
                            importance = as.numeric(table(om_cage_import))*om_roc$auc),
                          by = 'otu', all.x = T, suffixes = c('',round(om_roc$auc, 3)))
  om_all <- rbind(om_all, om_probs)
  om_aucs[j] <- om_roc$auc
}
om_roc <- roc(om_all$obs~om_all$pred)

raw_importance <- data.frame(om_import_otus[ , -c(1,2)], row.names = om_import_otus$otu)

importance_otus <-   apply(raw_importance, 1, sum, na.rm = T)
importance_otus <- importance_otus[importance_otus>0]
importance_otus <- importance_otus[importance_otus > median(om_aucs)*150/2]

### rank importance
importance_rank <- apply(raw_importance, 2, function(x) rank(x, na.last = T))
...
#### leave-2-out
LOdO_df <- Predict_df %>% 
                select(Euth_Early, human_source, one_of(euth_otu_feat))
...
plot(om_roc, col='red', lwd=2, add=T, lty=1)
...
legend('bottomright', legend=c(sprintf('Out-Of-Bag (AUC: %.3g)',otu_euth_roc$auc),
                               ...
                               sprintf('Leave-2 cages-out (AUC: %.3g)',LOdO_roc$auc),
...
),lty=c(1,2,2,2,1,0,0), lwd=2, col=c('black','blue','purple','red','gray'), bty='n')
```
...
for (i in seq_along(euth_otu_feat)) {
  partial_otu <- partialPlot(rf_early_aucrf$RFopt, aucrf_data, euth_otu_feat[i], xlab='',
                main=paste(get_tax(row_list = euth_otu_feat)$tax_label[i]))
