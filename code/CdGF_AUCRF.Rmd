...
dist_file <- '~/Documents/Github/Schubert_humanCdGF_XXXX_2016/data/mothur/gf_all.an.0.03.subsample.groups.summary'
entero_file <- '~/Desktop/dmm/human_CdGF.0.03.subsample.shared.for.0.03.dmm.2.mix.posterior'
...
dist_file <- read.table(dist_file, sep = '\t', header = T, row.names = 'group')
entero_file <- read.table(entero_file, sep = '\t', header = T, row.names = 'X')
source(tax_function)
...
rownames(entero_file) <- gsub('_','-', rownames(entero_file))
dist_file <- dist_file[grep('D0', rownames(dist_file)), ]
rownames(dist_file) <- gsub('D0', '-D0', rownames(dist_file))
rownames(dist_file) <- gsub('--', '-', rownames(dist_file))

Predict_df <- 
  merge(
    merge(
      merge(
        meta_file[meta_file$day %in% 2, c('mouse_id', 'log_cfu')],
        merge(meta_file[meta_file$day %in% 0, c('cage_id', 'mouse_id', 'Euth_Early')],
              entero_file, by = 'row.names'), by = 'mouse_id'), 
      dist_file, by.x = 'Row.names', by.y = 'row.names'),
    rel_abund_stool, by.x = 'mouse_id', by.y = 'row.names')

Predict_early_euth_df <- select(Predict_df, -cage_id, 
                                -mouse_id, -Row.names, -Partition_2,
                                -label, -X, -invsimpson_lci, -invsimpson_hci)
...
#### leave-one-cage-out
LOcO_df <- cbind(cage_id = sample(meta_file[meta_file$day %in% 0, 'cage_id'],
                                  nrow(Validate_early_euth_df)),
