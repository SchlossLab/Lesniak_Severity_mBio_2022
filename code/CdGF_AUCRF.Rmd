...
Predict_cage_df <- select(Predict_df, -human_source, -mouse_id, -Euth_Early)
Predict_source_df <- select(Predict_df, -cage_id, -mouse_id, -Euth_Early)
...
features_source <- head(sort(rf_source$importance[ , 'MeanDecreaseAccuracy'], decreasing = T), n_features)
features_cage <- head(sort(rf_cage$importance[ , 'MeanDecreaseAccuracy'], decreasing = T), n_features)
features_all <- unique(c(names(features_cage), names(features_source)))

# remove cage/inocula OTUs from prediction dataframes
Predict_early_euth_df <- select(Predict_df, -cage_id, -mouse_id, -human_source, -one_of(features_all))
...
#### leave-one-cage-out
LOcO_df <- Predict_df %>% 
                select(Euth_Early, cage_id, one_of(euth_otu_feat))
...
```{r importance, fig.width=10, fig.height=5}


#Importance Plot
#n_rf_features <- 9
euth_otus <- euth_otu_feat#[1:n_rf_features]
...
```{r partial dependence, fig.width=10, fig.height=10}
layout(matrix(c(1:9), nrow=3, byrow=TRUE))

partial_otu <- c()
for (i in seq_along(euth_otu_feat)) {
  partial_otu <- partialPlot(rf_early_aucrf$RFopt, aucrf_data, euth_otu_feat[i], xlab='',
                main=paste(euth_tax$tax_label[i]))
points(Predict_early_euth_df[Predict_early_euth_df$Euth_Early == 0 ,euth_otu_feat[i]], 
       jitter(rep(0.75*(diff(range(partial_otu$y))) + min(partial_otu$y), 
                  sum(Predict_early_euth_df$Euth_Early == 0)), 
              amount = (0.1*(diff(range(partial_otu$y))))), 
              pch = 21, bg = 'red')
points(Predict_early_euth_df[Predict_early_euth_df$Euth_Early == 1 ,euth_otu_feat[i]], 
       jitter(rep(0.25*(diff(range(partial_otu$y))) + min(partial_otu$y), 
                  sum(Predict_early_euth_df$Euth_Early == 1)), 
              amount = (0.1*(diff(range(partial_otu$y))))), 
              pch = 21, bg = 'gray')
}
