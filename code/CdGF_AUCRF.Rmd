...
for(j in 1:iters){
  set.seed(j)
  ran_aucrf_data <- cbind(Euth_Early = factor(sample(Predict_early_euth_df$Euth_Early, 
                                                     nrow(Predict_early_euth_df))),
                             Predict_early_euth_df[,rf_early_aucrf$Xopt])
  set.seed(seed)
  sampling <- sample(1:nrow(ran_aucrf_data),nrow(ran_aucrf_data),replace=F)
  ran_cv10f_probs <- rep(NA,nrow(ran_aucrf_data))
  for(i in seq(1,nrow(ran_aucrf_data),5)){
    train <- ran_aucrf_data[sampling[-(i:(i+4))],]
    test <- ran_aucrf_data[sampling[i:(i+4)],]
    temp_model <- AUCRF(Euth_Early~., data=train, pdel=0.99, ntree=500)
    ran_cv10f_probs[sampling[i:(i+4)]] <- predict(temp_model$RFopt, test, type='prob')[,2]
  }
  ran_cv10f_roc <- roc(ran_aucrf_data$Euth_Early~ran_cv10f_probs)
  ran_cv10f_all_pred <- c(ran_cv10f_all_pred, ran_cv10f_probs)
  ran_cv10f_all_resp <- c(ran_cv10f_all_resp, ran_aucrf_data$Euth_Early)
...
plot(LOcO_roc, col='red', lwd=2, add=T, lty=2)
plot(ran_cv10f_roc, col='orange', lwd=3, add=T, lty=2)
...
* When adding diversity and CFU, invsimpson and cfu appear as features, but when cfu is removed, invsimpson is no longer a predictor
...
random_boruta <- read.table('~/Documents/Github/Schubert_humanCdGF_XXXX_2016/data/process/random_rf_features_boruta.txt', sep = '\t')
random_boruta <- random_boruta[, c('normHits', 'decision')]
random_boruta$OTU <- rownames(random_boruta)
```

When randomly redistributing the Early Euthnization classification to the Day 0 commnunities, Boruta does not select any OTUs  
Summing the OTU decisions from 100 iterations of randomly assigning classifications run in Boruta with 100 iterations produces:
`r summary(random_boruta$decision)`
Of all the Confirmed OTUs there are `r length(unique(random_boruta$OTU[random_boruta$decision == 'Confirmed']))` unique OTUs
Of all the Tentative OTUs there are `r length(unique(random_boruta$OTU[random_boruta$decision == 'Tentative']))` unique OTUs
Furthermore, there is no overlap in the identified OTUs, since there are `r length(unique(random_boruta$OTU[random_boruta$decision %in% c('Tentative', 'Confirmed')]))` unique OTUs.  
Thus, no OTUs are able to create a model with out the specific pattern of observed disease severity.
...
# create RF model
rf_early_aucrf <- AUCRF(Euth_Early ~ ., data = Validate_early_euth_df,
                              ntree = n_trees, pdel = 0.05, ranking = 'MDA')

otu_euth_probs <- predict(rf_early_aucrf$RFopt, type = 'prob')
otu_euth_roc <- roc(Validate_early_euth_df$Euth_Early ~ otu_euth_probs[ , 2])

aucrf_data <- Validate_early_euth_df[,c('Euth_Early',rf_early_aucrf$Xopt)]
...
#### leave-one-cage-out
LOcO_df <- cbind(cage_id = Predict_df$cage_id,
                 Validate_early_euth_df[ , c('Euth_Early', rf_early_aucrf$Xopt)])
cage_list <- as.character(unique(LOcO_df$cage_id))
...
```{r Random_valid OTUs, fig.width=10, fig.height=8}
layout(matrix(c(1,1,1,1,4,4,4,
                2,2,2,5,3,3,3,
                2,2,2,5,3,3,3), nrow=3, byrow = TRUE))
...
#Abundance stripchart or most predictive otus
euth_otus <- rev(rf_early_aucrf$Xopt)
early_abunds <- Validate_early_euth_df[Validate_early_euth_df$Euth_Early==0, euth_otus]/10000 + 1e-4
late_abunds <- Validate_early_euth_df[Validate_early_euth_df$Euth_Early==1, euth_otus]/10000 + 1e-4
