...
#### leave-one-cage-out
LOcO_df <- merge(meta_file[meta_file$day == 0, c('mouse_id', 'cage_id', 'Euth_Early')], 
                               rel_abund_stool[ , rf_early_aucrf$Xopt], by.x = 'mouse_id', by.y='row.names')
cage_list <- unique(LOcO_df$cage_id)

LOcO_probs <- NULL
LOcO_probs <- sapply(1:length(cage_list), function(i){
  by_cage <- LOcO_df$cage_id == cage_list[i]
  LOcO_predict <- select(LOcO_df, -mouse_id, -cage_id)
...
### Validate AUCRF with Randomizing


```{r Random_validation, fig.width=6, fig.height=5, results= 'hide'}
#create dataframe for RF-classification
Euth_Early <- factor(sample(c(1,0), size = nrow(rel_abund_stool), replace = T))
Validate_early_euth_df <- cbind(Euth_Early, rel_abund_stool)
...
LOcO_df <- cbind(cage_id = sample(meta_file[meta_file$day %in% 0, 'cage_id']),
                 cbind(Euth_Early, rel_abund_stool[ , rf_early_aucrf$Xopt]))
cage_list <- unique(LOcO_df$cage_id)
...
legend('bottomright', legend=c(sprintf('Out-Of-Bag (AUC: %.3g)',otu_euth_roc$auc),
                               ...
                               sprintf('OOB vs Leave-1-out: p=%.2g', roc.test(otu_euth_roc,LOO_roc)$p.value),
                               sprintf('OOB vs 10-fold CV: p=%.2g', roc.test(otu_euth_roc,cv10f_roc)$p.value)
),lty=c(1,2,1,2,0,0), lwd=2, col=c('black','blue','gray','red'), bty='n')
```
