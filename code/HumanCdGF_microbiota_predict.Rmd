...
#### randomForest
```{r fig.width=10, fig.height=5}
meta_file$Euth_Early <- factor(ifelse(meta_file[ , 'Euthanized'] %in% 10, 1, 0)) 
...
rf_early_euth <- randomForest(Euth_Early ~ ., data = Predict_early_euth_df, 
                              importance = TRUE, ntree = n_trees)
rf_euth_import <- data.frame(importance = head(sort(rf_early_euth$importance[ , 'MeanDecreaseAccuracy'], decreasing = T), 10))

rf_early_euth_top_otu <- randomForest(Euth_Early ~ ., 
                                      data = Predict_early_euth_df[, c(rownames(rf_euth_import), 'Euth_Early')], 
                              importance = TRUE, ntree = n_trees)

unpredicted_top_otu <- Predict_early_euth_df_names[Predict_early_euth_df$Euth_Early != 
                                               rf_early_euth_top_otu$predicted]
