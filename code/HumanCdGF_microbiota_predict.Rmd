...
```{r, message=FALSE}
pack_used <- c('randomForest','miscTools','ggplot2','reshape2', 'pROC', 'knitr', 'cowplot')
...
setwd("~/Documents/Github/Schubert_humanCdGF_XXXX_2016/code")
...
meta_file <- meta_file[meta_file$sample_type == 'stool', ]
# remove unused cage P2 and 
meta_file <- meta_file[! meta_file$cage_id %in% 'P2', ]
...
all_rf_plot <- ggplot(data = RF_cfu_all_df, aes(x = log_cfu, y = predicted)) + geom_point() +
        ...
        annotate('text', x=0.15*(max(RF_cfu_all_df$log_cfu)), y=0.95*(max(RF_cfu_all_df$predicted)), 
                 label=paste('R^2 = ', tail(RF_cfu_all_rf$rsq, 1),'\nMSE = ', tail(RF_cfu_all_rf$mse, 1), sep = ''))
...
all_persist_rf_plot <- ggplot(data = RF_cfu_persist_df, aes(x = log_cfu, y = predicted)) + geom_point() +
        ...
        annotate('text', x=0.15*(max(RF_cfu_persist_df$log_cfu)), y=0.95*(max(RF_cfu_persist_df$predicted)), 
                 label=paste('R^2 = ', tail(RF_cfu_persist_rf$rsq, 1),'\nMSE = ', tail(RF_cfu_persist_rf$mse, 1), sep = ''))
...
mouse_list <- Predict_early_euth_df$Row.names
...
rf_early_euth <- randomForest(as.factor(Euth_Early) ~ ., data = Predict_early_euth_df, 
                              importance = TRUE, ntree = n_trees)
rf_euth_import <- data.frame(importance = head(sort(rf_early_euth$importance[ , 'MeanDecreaseAccuracy'], decreasing = T), 10))

rf_early_euth_top_otu <- randomForest(as.factor(Euth_Early) ~ ., 
                                      data = Predict_early_euth_df[, c(rownames(rf_euth_import), 'Euth_Early')], 
                              importance = TRUE, ntree = n_trees)

unpredicted_top_otu <- mouse_list[Predict_early_euth_df$Euth_Early != rf_early_euth_top_otu$predicted]
rf_euth_import_top_otu <- data.frame(importance = sort(rf_early_euth_top_otu$importance[ , 'MeanDecreaseAccuracy'], 
                                                       decreasing = T))
ROC_euth_top_otu <- roc(as.numeric(Predict_early_euth_df$Euth_Early), as.numeric(rf_early_euth_top_otu$predicted))
roc_plot_top_otu <- data.frame(Specificity = 1 - ROC_euth_top_otu$specificities, 
                       Sensitivity = ROC_euth_top_otu$sensitivities)

euth_rf_plot_top_otu <- ggplot(data = roc_plot_top_otu, aes(x = Specificity, y = Sensitivity)) + 
                      geom_path() + geom_abline(intercept = 0, slope = 1, 
                                                linetype = 'dashed', alpha = 0.5)
euth_import_plot_top_otu <- ggplot(data = rf_euth_import_top_otu, 
                           aes(x = factor(rownames(rf_euth_import_top_otu)), y = importance)) + 
        geom_point() + scale_x_discrete(limits = rev(rownames(rf_euth_import_top_otu))) +
        labs(x= '', y = '% Increase in MSE') + theme_bw() + coord_flip()
# title = 'OTU Importance in Prediction of Day Euthanized from Day 0 Community',

ggdraw()+
  draw_plot(euth_rf_plot_top_otu, 0, 0, 0.5, 1) + 
  draw_plot(euth_import_plot_top_otu, 0.5, 0, 0.5, 1)
```

Confusion Matrix  
`r kable(rf_early_euth_top_otu$confusion)`

OOB error rate = `r 100*tail(rf_early_euth_top_otu$err.rate[ , 1], 1)`

Mice which were euthanized early, but the model predicts the will were not (Mouse Tag - Cage):
`r unpredicted_top_otu`
...
```{r fig.width=10, fig.height=5}
layout(matrix(rep(c(1,1,1,1,2,2,2,2),4), ncol=8, nrow = 4, byrow = T))
