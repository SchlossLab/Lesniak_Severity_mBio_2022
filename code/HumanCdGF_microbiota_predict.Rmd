...
# remove unused cage P2 and non 431 cdiff
meta_file <- meta_file[! meta_file$cage_id %in% c('P2', 'CON', 'CON2', 'CON3'), ]
...
unpredicted <- mouse_list[Predict_early_euth_df$Euth_Early != rf_early_euth$predicted]
rf_euth_import <- data.frame(importance = head(
  sort(rf_early_euth$importance[ , 'MeanDecreaseAccuracy'], decreasing = T), 30))
ROC_euth <- roc(as.numeric(Predict_early_euth_df$Euth_Early), as.numeric(rf_early_euth$predicted))
roc_plot <- data.frame(Specificity = 1 - ROC_euth$specificities, Sensitivity = ROC_euth$sensitivities)

euth_rf_plot <- ggplot(data = roc_plot, aes(x = Specificity, y = Sensitivity)) + geom_path() + 
  geom_abline(intercept = 0, slope = 1, linetype = 'dashed', alpha = 0.5)
euth_import_plot <- ggplot(data = rf_euth_import, aes(x = factor(rownames(rf_euth_import)), y = importance)) + 
        geom_point() + scale_x_discrete(limits = rev(rownames(rf_euth_import))) +
        labs(x= '', y = '% Increase in MSE') + theme_bw() + coord_flip()
# title = 'OTU Importance in Prediction of Day Euthanized from Day 0 Community',

ggdraw()+
  draw_plot(euth_rf_plot, 0, 0, 0.5, 1) + 
  draw_plot(euth_import_plot, 0.5, 0, 0.5, 1)
```

Confusion Matrix  
`r kable(rf_early_euth$confusion)`

OOB error rate = `r 100*tail(rf_early_euth$err.rate[ , 1], 1)`
