...
```{r, message=FALSE}
pack_used <- c('randomForest','miscTools','ggplot2','reshape2', 'pROC', 'knitr')
...
ggplot(rsq_features, aes(x=Features..n., y=RSQ, group=(Day), color=factor(Day))) + geom_line() +
  labs(title= 'R^2 from RF with top n features\n
...
ggplot(data = rsq_persist, aes(x = factor(day), y = RSQ)) + geom_point() + theme_bw() +
        labs(title = 'Variation Explained by Day of C Diff CFU dpi from Community on Day 0
                      \nwithout mice euthanized early', x= 'Day', y = 'R^2')
...
ggplot(data = mse_persist, aes(x = factor(day), y = MSE)) + geom_point() + theme_bw() +
        labs(title = 'MSE by Day of C Diff CFU dpi from Community on Day 0\n
             without mice euthanized early', x = 'Day', y = 'MSE')

ggplot(features_persist, aes(x=Features..n., y=RSQ, group=(Day), color=factor(Day))) + 
  geom_line() + labs(title= 'R^2 from RF with top n features\n
...
ggplot(data = med_import_df, aes(x = factor(Day), y = Importance)) + 
...
ggplot(data = rf_euth_import, aes(x = factor(rownames(rf_euth_import)), y = importance)) + 
        geom_point() + scale_x_discrete(limits = rev(rownames(rf_euth_import))) +
        labs(title = 'OTU Importance in Prediction of Day Euthanized from Day 0 Community',
        x= '', y = '% Increase in MSE') + theme_bw() + coord_flip()
```
