...
RandomForest_by_day <- function(i, community){
if(community == day_0){
  rf_data <- 
  meta_file %>%
  filter(day == i) %>% 
  ...
  print('Input for community needed (day_0 or by_day')
}
...
rf_day_0_comm_plots <- lapply(1:10, day_0)
rf_by_day_com_plots <- lapply(1:10, by_day)
