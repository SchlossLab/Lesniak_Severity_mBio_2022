...
meta_file <- 
  ...
  select(Mouse_ID, group, day, cdiff_cfu, Euthanized) %>% 
...
rel_abund_shared_01$group <- rownames(rel_abund_shared_01)
...
cfu_by_day_comm <-
  meta_file %>% 
  filter(day %in% 1:10) %>% 
  select(group, day, log_cfu) %>% 
  left_join(rel_abund_shared_01) %>% 
  select(-group, -one_of(cdiff_otu))
...
### Otus associated with CFU C difficile
...
predictive_otus <- paste('Otu00', c('199','026','050','015','028',
                         ...
                         sep = '')
cfu_by_day_comm %>% 
  select(log_cfu, one_of(predictive_otus)) %>% 
  filter(!is.na(log_cfu)) %>% 
  gather('otu','abundance',contains('Otu')) %>% 
  ggplot(aes(x = abundance, y = log_cfu)) + geom_point() + 
...
cfu_by_day_comm %>% 
  ...
  filter(!is.na(log_cfu)) %>% 
  gather('otu','abundance',contains('Otu')) %>% 
  ggplot(aes(x = log10(abundance), y = log_cfu)) + geom_point() + 
    facet_wrap(~ otu, scales = 'free_x') +
