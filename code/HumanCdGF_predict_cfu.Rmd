...
rf_bor_import <- sort(importance(rf_boruta)[,1], decreasing = T)
detach('package:Boruta', unload = T)
detach('package:ranger', unload = T)
...
reduced_otus <- otus_used[[6]]

### test models with top subset of otus
top_important_OTU <- reduced_otus
...
one_mouse_df <- data.frame(colonized_cfu %>% 
    right_join(community_d0.df) %>% 
    filter(!is.na(log_cfu_med))  %>% 
    select(log_cfu_med, mouse_id, cage_id, Otu00250, Otu00005))
