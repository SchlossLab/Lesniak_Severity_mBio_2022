...
plot_loop_output <- lapply(seq_along(importance_om$tax), function(i){
    partialPlot(rf_n_features, top_colonized_df, rownames(importance_om)[i],
                xlab='', main=paste(importance_om$tax_label[i]))
    par(new = T)
    plot(y = top_colonized_df[ , 'log_cfu_med'],
         x = top_colonized_df[ , rownames(importance_om)[i]], 
...
# To color points by mouse - Change pch to 21 and un-comment following lines
  lapply(seq_along(unique(partial_df$cage_id)), function(m){
    ...
    points(y = partial_df[partial_df$cage_id %in% mouse, 'log_cfu_med'],
           x = partial_df[partial_df$cage_id %in% mouse , rownames(importance_om)[i]], 
