...
```{r predict median CFU, fig.width=6,fig.height=6}
...
cfu_shared_colonized.df <- colonized_cfu %>% 
    ...
    select(-mouse_id, -cage_id, -one_of(features_all)) %>% 
    filter(!is.na(log_cfu_med))

test <- Boruta(log_cfu_med ~ ., data = cfu_shared_colonized.df, maxRuns = 5000)

cfu_shared_colonized <- cfu_shared_colonized.df %>% 
  select(log_cfu_med, one_of(names(test$finalDecision[test$finalDecision %in% c('Confirmed', 'Tentative')])))

set.seed(seed)
rf_all <- randomForest(log_cfu_med ~ ., data = cfu_shared_colonized.df,
                           importance = TRUE, ntree = n_trees, mtry = 14)
set.seed(seed)
rf_bor <- randomForest(log_cfu_med ~ ., data = cfu_shared_colonized,
                           importance = TRUE, ntree = n_trees, mtry = 2)

plot(importance_sorted/max(importance_sorted))
importance_sorted <- sort(importance(rf_bor)[,1], decreasing = T)
top_important_OTU <- rownames(data.frame(importance_sorted[importance_sorted/max(importance_sorted) > 0.6]))

set.seed(seed)
rf_n_features <- randomForest(log_cfu_med ~ ., data=cfu_shared_colonized.df %>% 
...
otu_taxa <- get_tax(1, importance.df$otu, tax_file)

importance_plot_day <- 
  ggplot(data = importance.df, aes(x = factor(otu), y = importance)) + 
        geom_point() + scale_x_discrete(limits = rev(importance.df$otu),
