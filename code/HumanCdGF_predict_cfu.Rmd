...
shared_file <- 'data/process/humanCdGF_full.shared'
...
rel_abund_shared <- 100 * shared_file / apply(shared_file, 1, sum)
otu_presence <- ifelse(rel_abund_shared >= (1/2000), T, F) 
rel_abund_shared_01 <- rel_abund_shared[ , apply(otu_presence, 2, sum) > round(otu_presence_cutoff*nrow(rel_abund_shared))]
    ...
    if(length(features_cor[[otu]]) > 1){
      ...
      mtext(side = 1, line = 6, cex = 0.6,
        paste('Correlated features: \n', 
              paste(paste0(get_tax(row_list = features_cor[[otu]])$tax_label, c(', ', ',\n ')), 
                    collapse = '')))
      } else {
...
for (otu in seq_along(cr_features)) {
    ...
    if(length(cr_features_cor[[otu]]) > 1){
      ...
      mtext(side = 1, line = 6, cex = 0.6,
        paste('Correlated features: \n', 
              paste(paste0(get_tax(row_list = cr_features_cor[[otu]])$tax_label, c(', ', ',\n ')), 
                    collapse = '')))
      } else {
