...
```{r setup environment}
pack_used <- c('randomForest','miscTools','tidyr','ggplot2','dplyr', 'pROC', 'knitr', 'cowplot', 'caret')
...
rel_abund_01_filtr <- rel_ab_otu_df[,-cor_otus]
...
importance_om <- get_tax(row_list = unlist(features_cor))

import_rel_ab <- rel_abund_shared_01[ , rownames(importance_om)]
import_rel_ab$sample_id <- rownames(import_rel_ab)
partial_df <- data.frame(import_rel_ab %>%
    right_join(sample_list_d0) %>% 
    select(-sample_id) %>%
    left_join(colonized_cfu) %>% 
    filter(!is.na(log_cfu_med)))
