...
set.seed(seed)
rf_model <- rfe(cfu_shared_colonized.df[ , -1], cfu_shared_colonized.df$log_cfu_med,
                 sizes = subsets,
                 rfeControl = rfe_ctrl)
rf_test <- train(cfu_shared_colonized.df[ , importance_final], cfu_shared_colonized.df$log_cfu_med,
                 method = 'rf', importance = T)
partialPlot(rf_test$finalModel, cfu_shared_colonized.df, 'Otu000004')


xrf_test <- randomForest(log_cfu_med ~ ., data = cfu_shared_colonized.df[ , 1:30],
                     importance = TRUE, ntree = 100)

importance_final <- predictors(rf_model)

rf_n_features <- rf_model$fit
