...
LOcO_df <- data.frame(colonized_cfu %>% 
    right_join(community_d0.df) %>% 
    filter(!is.na(log_cfu_med)) %>% 
    select(log_cfu_med, cage_id, one_of(top_important_OTU)))
...
for(i in 1:length(cage_list)){
  ...
  set.seed(seed)
  temp_model <- randomForest(log_cfu_med~., data=train, pdel=0.99, ntree=500)
  import_df <- importance(temp_model)
  colnames(import_df) <- cage_list[i]
  import_otus <- merge(import_otus, import_df, by.x = 'otu', by.y = 'row.names')
  rsq[i] <- round(tail(temp_model$rsq, 1), 2)
  LOcO_probs <- rbind(LOcO_probs, data.frame(cage=rep(cage_list[i], nrow(test)),
                                             actual=test$log_cfu_med, 
                                             pred=predict(temp_model, test)))
}
...
ggplot(data = rf_prediction, aes(x=observed, y=predicted, color = cage_id)) + geom_point() + 
  ...
  theme_bw() + annotate('text', x=0.15*(max(rf_prediction$observed)), 
