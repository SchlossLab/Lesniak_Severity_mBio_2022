...
for(i in 1:iters){
    ...
    for(i in 4:ncol(train)){
      pp_var <- data.frame(otu = colnames(train)[i],
                           partialPlot(rf_om, data.frame(select(train, -mouse_id, -cage_id)), 
                                       x.var= colnames(train)[i], plot = FALSE))
      om_pp <- rbind(om_pp, pp_var)
      }
...
OTUS <- unique(as.character(om_pp$otu))
part_plot_cfu <- c()
for(i in OTUS){
om_v_oob <- rbind(data.frame(model = 'OM',
                             om_pp[om_pp$otu == i, ]),
                  data.frame(model = 'OOB', otu = i,
                             partialPlot(rf_n_features, data.frame(cfu_shared_colonized.df), 
                                         OTUS[OTUS == i], plot = F)))
gg_pp <- ggplot(om_v_oob, aes(x,y, group = model, color = model)) +geom_smooth() + ggtitle(i)
part_plot_cfu[[i]] <- gg_pp
}
library(cowplot)

ggdraw() + 
  draw_plot(part_plot_cfu[[1]])
plot_grid(part_plot_cfu[[1]], part_plot_cfu[[2]])

do.call('plot_grid',part_plot_cfu)
i <- 'Otu000004'
plot(om_pp$x[om_pp$otu == i], om_pp$y[om_pp$otu == i])
lines(lowess(om_pp$x[om_pp$otu == i], om_pp$y[om_pp$otu == i]))
library(car)
scatterplot(y~x, om_pp[om_pp$otu == i, ], spread = F, smoother = gamLine,
            grid = F, lty = 0)

# partial plot for one_mouse model
#ggplot(data=om_pp[om_pp$otu==names(final_rank[5]), ], aes(x=x, y=y)) + geom_smooth()
...
for(i in 1:iters){
    ...
    for(i in 4:ncol(train)){
      pp_var <- data.frame(otu = colnames(train)[i],
                           partialPlot(rf_om, data.frame(select(train, -mouse_id, -cage_id)), 
                                       x.var= colnames(train)[i], plot = FALSE))
      om_pp_cr <- rbind(om_pp_cr, pp_var)
      }
...
om_v_oob <- rbind(data.frame(model = 'OM',
                             om_pp_cr[om_pp_cr$otu == 'Otu000004', ]),
                  data.frame(model = 'OOB', otu = 'Otu000004',
                             partialPlot(rf_col_rate, shared_cr_df, 'Otu000004',
                                         plot = F)))
ggplot(om_v_oob, aes(x,y, group = model, color = model)) +geom_smooth() + geom_point()
