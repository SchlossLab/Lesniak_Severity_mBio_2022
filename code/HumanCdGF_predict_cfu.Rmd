...
# plot initial C. difficile colonization of mice 
#meta_data %>% 
...
#  select(ratio, one_of(cr_features)) %>% 
#  left_join(col_rate))
...
ggplot(data = cr_prediction, aes(x=observed, y=predicted, color = cage_id)) + geom_point() + 
  ...
  annotate('text', x=0.15*(max(cr_prediction$observed)), 
           y=0.95*(max(cr_prediction$predicted)), parse = T,
           label=paste("R^2 == ", round(tail(rf_col_rate$rsq, 1), 2))) # + 
