...
```{r setup environment}
pack_used <- c('randomForest','tidyr','ggplot2','plyr', 'dplyr', 'caret', 'vegan')
...
colonized_cfu_range <- meta_data %>% 
  filter(day %in% c(4:10)) %>% 
  group_by(mouse_id, cage_id, human_source) %>% 
  summarise(log_cfu_med = median(log_cfu),
            low_iqr = quantile(log_cfu)['25%'],
            high_iqr = quantile(log_cfu)['75%']) %>% 
  ungroup()
...
for(i in 1:100){
  ...
  set.seed(seed)
  randomized_wo_hilo_rf <- randomForest(select(cfu_wo_hilo_df, contains('Otu')), 
                                        random_cfu, ntree = 501, mtry = 15)
...
set.seed(seed)
cfu_wo_hilo_rf <- randomForest(select(cfu_wo_hilo_df, contains('Otu')), 
                               cfu_wo_hilo_df$log_cfu_med, ntree = 1001,
                               mtry = 15, importance = T)
...
```{r IQR of CFU in middle range, fig.height=4, fig.width=5}
meta_data %>% 
  filter(day %in% c(4:10)) %>% 
  group_by(mouse_id, cage_id, human_source) %>% 
  merge(data.frame(colonized_cfu %>% 
    select(mouse_id, human_source, log_cfu_med) %>% 
    right_join(community_all_d0) %>% 
    filter(!human_source %in% c('DA00581', 'DA01134','DA01146')) %>% 
    filter(!is.na(log_cfu_med)) %>% 
      select(mouse_id), 
                   predicted = cfu_wo_hilo_rf$predicted)) %>% 
  ggplot(aes(x=predicted, y=log_cfu, group=mouse_id))+
     stat_summary(fun.data = 'median_hilow', fun.args=(conf.int=0.5)) + 
     xlim(7,8) + ylim(5,9) +
     labs(x= "Predicted CFU (log10)", y='Observed CFU (log10)\nMedian with IQR of CFU from day 4-10') +
     theme(panel.background = element_rect(linetype = 1, size = 1, fill = 'white', color = 'black'),
          panel.grid.major.x = element_blank(),
          panel.grid.minor.x = element_blank(),
          panel.grid.major.y = element_blank(),
...
```{r NMDS of Colonization Level, fig.height=6, fig.width=6}
setwd('~/Documents/Github/Schubert_humanCdGF_XXXX_2016/')
full_shared_file <- 'data/process/full_shared.shared'
full_shared_file <- read.table(full_shared_file, sep = '\t', header = T)
...
nmds_group <- colonized_cfu %>% 
    select(mouse_id, human_source, log_cfu_med) %>% 
    right_join(community_all_d0) %>% 
    filter(!is.na(log_cfu_med)) %>% 
    mutate(colonization_level= ifelse(log_cfu_med >= 8.25, 'hi',
                                     ifelse(log_cfu_med <= 6.5, 'lo', 'mid'))) %>% 
    select(colonization_level, mouse_id)

rownames(dist_matrix) <- gsub('_D0','',rownames(dist_matrix))
colnames(dist_matrix) <- gsub('_D0','',colnames(dist_matrix))
...
plot(nmds, type = 'n')
points(nmds, display = 'sites', pch = 21, bg = colors[as.numeric(factor(nmds_group$colonization_level))])
legend('topright', c('High (> 8.25 Log10 CFU)','Middle (8.25-6.5 Log10 CFU)','Low (< 6.5 Log10 CFU)'), 
       pch =21, pt.bg=c('red','orange','green'), inset=0.09, box.lty = 0)
#adonis(dist_matrix~colonization_level, data = nmds_group)
```
