...
```{r plot colonization otu correlations, fig.width=10, fig.height=10}
...
subset(cfu_all_shared_df, select = 
  ...
  ggplot(aes(x = abundance, y = log_cfu_med)) + geom_point() + 
    ...
    ylim(5, 9) + scale_x_log10(breaks = c(0, 0.01, 0.1, 1, 10, 100)) +
    theme(strip.background = element_rect(colour=NULL, fill='white'),
          ...
          panel.grid.minor.x = element_line(linetype = 1, size = .1, color = 'gray'))
```
...
```{r plot source otu correlations, fig.width=10, fig.height=10}
