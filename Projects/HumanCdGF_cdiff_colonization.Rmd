...
So is the performance of the whole model appears to be dependent on two grouping of points (one around ~5-6 log and one around ~8 log ). So how well can random forest model the data with out the DA00581 source?
...
Looking at the % var explained values from each individual iteration, there appears to be a bimodal distribution in the performance.  
...
```{r compare number of 581 donor cages, fig.height=5, fig.width=5}
...
rbind(data.frame(rsq = om_rsq_pos_df$rsq[om_rsq_pos_df$data_set=='test'], data = 'all'),
      rbind(data.frame(rsq = om_rsq_AD_df$rsq[om_rsq_AD_df$data_set=='test'], data = 'Two_only'),
            data.frame(rsq = om_rsq_C_only_df$rsq[om_rsq_C_only_df$data_set=='test'], data = 'One_only'))) %>% 
  ggplot(aes(rsq, color = data)) + 
  geom_density() + ggtitle('Rsq Distribution when multiple cages from same Donor present')
```


It seemed possible the assumption all donors have the same community may be false. However, when running the analysis with only one cage from the donor with multiple cages it performs as we observe for the lower peak in the bimodal distribution and when a second cage from the same donor is added we observe Rsq at the higher peak in the bimodal distribution. Thus there appears to be a cage effect improving the performance and the bimodal distribution maybe due to a variation in the ability of these communities to be able to predict each other, ie some be more similar to each other but still as a group more different than the other cages.
