...
for(i in 1:iters){
    ...
    om_rsq <- rbind(om_rsq, c(paste(train$mouse_id, collapse = ' '), 
                              tail(rf_om$rsq,1),
                              1 - (sum((test$log_cfu_med - predict(rf_om, test))^2)/
...
om_otu_rank <- om_otu_rank*as.numeric(om_rsq[,3])
