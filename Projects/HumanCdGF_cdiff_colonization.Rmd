...
#### Predictive OTUs  
##### OTUs predictive of _Colonization level_:  
`r names(top_features)`  
...
##### OTUs predictive of _Inocula Source_:  
`r names(top_features_src)`  
...
subset(cfu_all_shared_df, select = 
       c("log_cfu_med", names(feat_cor_cfu)[feat_cor_cfu>0.5 & feat_cor_cfu < 1])) %>% 
  gather(OTU, abundance, contains('Otu')) %>% 
  ggplot(aes(x = abundance, y = log_cfu_med)) + geom_point() + 
    facet_wrap(~OTU, ncol = 4, scales = 'free_x')
```
