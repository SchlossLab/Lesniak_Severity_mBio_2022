...
histology %>% 
  ...
  ggplot(aes(x = histology, y = score, color = outcome)) +
...
toxin %>% 
  ...
  ggplot(aes(x = day, y = Log_repiricoal_dilution, color = outcome)) + 
...
abundance_plot <- metadata %>% 
	...
	group_by(mouse_id, group, Genus, order, early_euth) %>% 
  ...
  highlight_key(~mouse_id) %>% 
	ggplot(aes(x = reorder(tax_otu_label, order), 
			y = abundance, color = as.factor(early_euth), shape = mouse_id)) + 
