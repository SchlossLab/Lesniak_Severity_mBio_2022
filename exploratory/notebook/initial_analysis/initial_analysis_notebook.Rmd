...
metadata %>% 
  ...
  mutate(cfu = log10(cdiff_cfu)) %>% 
  pivot_longer(cols = c(edema_tissue, inflammation_tissue, epithelial_damage, summary_score),
               names_to = 'hist_score', values_to = 'score') %>% 
  group_by(hist_score) %>% 
  nest() %>% 
	mutate(spearman_p = map(.x = data, .f = ~ cor.test(.x$cfu, .x$score, 
	                                                   method = 'spearman', exact = F)$p.value),
	       spearman_rho = map(.x = data, .f = ~ cor.test(.x$cfu, .x$score, 
	                                                     method = 'spearman', exact = F)$estimate)) %>%
	unnest(c(spearman_p, spearman_rho)) %>% 
	mutate(pvalue = p.adjust(spearman_p, method = 'BH', n = 5)) %>% 
	unnest(data) %>% 
  ggplot(aes(x = cfu, y = score)) + 
    ...
    facet_wrap(hist_score~., scale = 'free_y')
...
Correlations between CFU and histology score not significant after BH correction.
  ...
  shared %>% 
    ...
    #highlight_key(~mouse_id) %>% 
      ...
      theme(panel.grid.major.y = element_blank(), 
  			...
  			legend.position = 'bottom') + 
  		coord_flip() + 
      geom_jitter(position = position_jitterdodge(dodge.width = 0.5), alpha = 0.2)
}
...
## Toxin presence for mild outcome
```{r}
plot_lefse_data('toxin_mild')
```



## Toxin presence for severe outcome
```{r fig.height = 4}
plot_lefse_data('toxin_moribund')
```



## Toxin presence and outcome severity
```{r, fig.height = 12}
plot_lefse_data('toxin_severity')
```
